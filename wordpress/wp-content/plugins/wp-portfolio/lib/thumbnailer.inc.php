...
function WPPortfolio_checkWebsiteThumbnailCaptured($url, $args = null)
{
	...
	$detail = sprintf('<span class="wpp-debug-detail-header">Error Summary</span>
                      ...
                      <span class="wpp-debug-detail-header">Request Status Code</span>
                      <span class="wpp-debug-detail-info">%s</span>
...
function WPPortfolio_xml_processReturnData($data)
{
	...
	else 
	{
		...
		// Need feature update - e.g. inside thumbs
            ...
            $returndata['status'] = 'other_error';
            $returndata['msg'] = 'Could not parse response using '.$parseMethod;
		}
 ...
 * @param $localPath The path to use to store the image locally.ы
 */
function WPPortfolio_downloadRemoteImageToLocalPath($remoteURL, $localPath)
{
	...
	if ($imagedata)
	{
        ...
        elseif (is_admin())
        {
            WPPortfolio_showMessage(__("Sorry, but an unknown error occurred while attempting to write to the cache directory. Check your <a href = '" . WPP_SETTINGS . "'>Portfolio Settings</a>.", 'wp-portfolio'), true);
        }
    }
    else
    {
